...
def verificationandstuff():
    ...
    while not verified:
        ...
        if(usr_veri != str and usr_veri.json()["status"] == "not_verified"):
            ...
            if(verify_usr.json()["status"] == "already_verified"):
                config.set_user_key(verify_usr.json()["userKey"])
...
def generate(prompt, style_prompt):
    #prompt = askprompt()

    from style import style

    prompt += " " + style[style_prompt][0]
    neg_prompt = style[style_prompt][1]

    print(prompt, neg_prompt)

    gen_img = calls.generate_img(config.GENERATOR_API, config.get_generator_payload(prompt, neg_prompt, config.USER_KEY, config.ADACCESSCODE))
...
def saveimageandshow(image_data, show = False):
    if image_data == None:
        return

    fullpath = "static/gen_img.jpeg"
    ...
    with open(fullpath, "wb") as file:
        file.write(image_data[1].content)
        #load(10, "Saving ")
        write(f"Saved log as {image_data[0]}", GOOD)
...
def gen_img(prompt, style):
    saveimageandshow(generate(prompt, style))
